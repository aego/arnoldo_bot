...
class BotConfiguration(
    ...
    // @Bean
    fun bot(): TelegramBotsApi {
        return TelegramBotsApi(session().javaClass)
    }

    private fun session(): ArnoldoSession {
        val session = ArnoldoSession()
        session.setToken(properties.token)
        return session;
    }
}
